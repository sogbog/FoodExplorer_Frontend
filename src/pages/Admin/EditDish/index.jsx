...
export function EditDish(){
    ...
    useEffect(() =>{
        async function fetchInfo(){
            ...
            setIngredients(dishResponse.data.ingredients.split(", "))
            setCategories(categoriesResponse.data)
            setName(dishResponse.data.name)
            setPrice(dishResponse.data.price)
            setDescription(dishResponse.data.description)
            setCategory(dishResponse.data.category_id)
            setLoading(false)
        }
        fetchInfo()
    }, [])
    ...
    async function handleUpdate(){
        try {
            if(avatarFile){
                ...
                await api.patch(`/dishes/${dish.id}/avatar`, fileUploadForm)
            }
            ...
            await api.put(`/dishes/${dish.id}`, newDishInfo)
            alert("Prato atualizado com sucesso!")
            navigate(`/dish_view/${dish.id}`)
    ...
    async function handleDelete(){
        const confirm =  window.confirm("Tem certeza que deseja deletar esse prato?")
    ...
    function setDefaultCategory(){
        const [cat] = categories.filter(category => category.id == dish.category_id)
    ...
    function handlePrice(event){
        ...
        if (price == ""){
            ...
            switch (price.toString().length) {
                case 1: displayPrice = ("R$ 00,0" + price.toString()); break
                
                case 2: displayPrice = ("R$ 00," + price.toString()); break
    
                case 3: displayPrice = ("R$ 0" + price.toString().replace(/\B(?=(\d{2}$))/, ',')); break
    
                default: displayPrice = ("R$ " + price.toString().replace(/\B(?=(\d{2}$))/, ',')); break
            }
        ...
        <Container>
            ...
            <InfoArea>
                <DesktopWrapper>
                    <InputWrapper id="uploadWrapper">
                        ...
                        <label htmlFor="imgUpload" id="uploadView">
                            ...
                            <span id="uploadText">Selecione uma imagem para alterá-la</span>
                        </label>
                    </InputWrapper>
                    ...
                    <InputWrapper id="categoryWrapper">
                        ...
                        <label htmlFor="category" id="selectWrapper">
                            ...
                            <select id="category" defaultValue={setDefaultCategory()} onChange={e => setCategory(e.target.value)}>
                ...
                <div id="buttonWrapper">
                    <Button colorCode='DARK_800' type="button" onClick={handleDelete}>Excluir prato</Button>
                    <Button colorCode='TOMATO_400' type="button" onClick={handleUpdate}>Salvar</Button>
                </div>
            </InfoArea>}
